...
export default function ProductConfigurator({ productSlug, workerUrl = 'https://hercules-product-sync-live.gilles-86d.workers.dev' }: ProductConfiguratorProps) {
                        ...
                        >
                          {textPos === 'next_to' ? (
                            ...
                            <>
                              <div className="kd-image-selector-title" style={{ fontWeight: weightMap[textWeight] || 500 }}>{option.name}</div>
                              {option.image && (
                                <img src={option.image} alt={option.name} style={{ objectFit: 'contain', margin: '6px 0', ...getAddonIconStyle(option, addon, { sides: true, bottom: true }) }} />
                              )}
                            </>
                          )}
                        </div>
              ...
              <div className="range-wrapper">
                ...
                {tempQuantity > quantityRange.max && (
                  <p className="kd-qty-warning">
                    Für diese Menge kontaktieren Sie uns bitte, und wir erstellen Ihnen ein individuelles Angebot.
                  </p>
                )}
              </div>
            </div>
