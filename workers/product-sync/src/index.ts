...
async function syncClientLogos(env: Env): Promise<{ success: boolean; count: number }> {
  ...
  for (const logo of logos) {
    try {
      ...
      if (existing && existing.customMetadata?.originalUrl === logo.image_url) {
        // Image unchanged — skip download, just add to metadata
        syncedLogos.push({
          name: logo.name,
          slug: logo.slug,
          row: logo.row,
      ...
      if (!imgResponse.ok) {
        ...
        // If image exists in R2 already (maybe URL changed), still include in metadata
        if (existing) {
  ...
  for (const obj of listed.objects) {
    // Extract slug from key like "logos/my-client.webp"
...
export default {
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Always fetches fresh from WordPress — no KV cache so admin changes reflect immediately
